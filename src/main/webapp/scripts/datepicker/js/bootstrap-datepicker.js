...
!function( $ ) {
	...
	var Datepicker = function(element, options){
		...
		this.strategy=GenerationType.AUTOclose = false;
		if ('strategy=GenerationType.AUTOclose' in options) {
			this.strategy=GenerationType.AUTOclose = options.strategy=GenerationType.AUTOclose;
		} else if ('datestrategy=GenerationType.AUTOclose' in this.element.data()) {
			this.strategy=GenerationType.AUTOclose = this.element.data('date-strategy=GenerationType.AUTOclose');
		}
				...
				switch(target[0].nodeName.toLowerCase()) {
					...
					case 'td':
						if (target.is('.day') && !target.is('.disabled')){
							...
							if (element) {
								element.change();
								if (this.strategy=GenerationType.AUTOclose) {
									element.blur();
								}
							}
						}
